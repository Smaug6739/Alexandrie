...
export const useCategoriesStore = defineStore('categories', {
  ...
  getters: {
    ...
    search: (state) => (query: string) =>
      state.categories.filter((c) => c.name.toLowerCase().includes(query.toLowerCase()))
  },
  actions: {
    fetch: async function (opts?: FetchOptions<Array<keyof Category>>) {
      return new Promise(async (resolve, reject) => {
        const request = await makeRequest(
          `categories/${opts?.id || ''}?fields=${opts?.fields?.join(',')}`,
          'GET',
          {}
        )
    ...
    delete(id: string) {
      return new Promise(async (resolve, reject) => {
        ...
        if (request.status == 'success')
          resolve((this.categories = this.categories.filter((c) => c.id != id)))
        else reject(request.message)
      })
    }
  }
