...
export const useDocumentsStore = defineStore('documents', {
  ...
  getters: {
    ...
    getByCategories: (state) => (category: string) =>
      state.documents.filter((d) => d.category == category),
    search: (state) => (query: string) =>
      state.documents.filter((d) => d.name.toLowerCase().includes(query.toLowerCase())),
  ...
  actions: {
    fetch: async function <T extends FetchOptions<Array<keyof Document>>>(
      ...
      return new Promise(async (resolve, reject) => {
        ...
        console.log(`[store/documents] Fetch request status: ${request.status}`)
    ...
    update(doc: Document) {
      return new Promise(async (resolve, reject) => {
        ...
        if (request.status == 'success')
          resolve((this.documents = this.documents.map((d) => (d.id == doc.id ? doc : d))))
        else reject(request.message)
      })
    },
    delete(id: string) {
      return new Promise(async (resolve, reject) => {
        ...
        if (request.status == 'success')
          resolve((this.documents = this.documents.filter((d) => d.id != id)))
        else reject(request.message)
      })
    }
  }
