<template>
  <aside class="sidebar" :class="isOpened ? 'open' : ''">
    <section class="header">
      <RouterLink to="/" class="name" @click="isOpened = false">
        <Icon
...
watch(isOpened, (val: boolean) => {
  if (val && isMobile()) document.getElementById('backdrop')?.classList.add('backdrop');
  if (!val && isMobile()) document.getElementById('backdrop')?.classList.remove('backdrop');
});

const menuItems = computed((): MenuItem[] => {
  const items: MenuItem[] = [];
  const category = categoriesStore.getById(route.params.category_id as string);
