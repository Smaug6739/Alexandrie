...
export default class CategoriesClass extends Base {
  constructor(app: App) {
    super(app);
  }
  ...
  public getAll() {
    return new Promise((resolve, reject) => {
      this.app.db.query<Theme[]>('SELECT * FROM main_categories ORDER BY `order`', [], (err, themes) => {
        if (err) return reject('Internal database error.');
        this.app.db.query<Category[]>('SELECT * FROM sub_categories ORDER BY `order`', [], (err, categories) => {
          if (err) return reject('Internal database error.');
  ...
  updateMainCategory(id: string, name: string, description: string, order: number, path: string, icon: string) {
    return new Promise((resolve, reject) => {
      ...
      this.app.db.query<Theme[]>('SELECT * FROM main_categories WHERE id = ? LIMIT 1', [id], (err, result) => {
        if (err) return reject('Internal database error.');
  ...
  public updateSubCategory(
    ...
    return new Promise((resolve, reject) => {
      if (!id) return reject(new Error('[MISSING_ARGUMENT] : id must be provided'));
      this.app.db.query<Category[]>('SELECT * FROM sub_categories WHERE id = ? LIMIT 1', [id], (err, result) => {
        if (err) return reject('Internal database error.');
        if (!result || !result[0]) return reject(new Error('[ERROR] : Invalid id'));
        else {
          if (!name) name = result[0].name;
  ...
  public deleteMainCategory(id: string) {
    return new Promise((resolve, reject) => {
      if (!id) return reject(new Error('[MISSING_ARGUMENT] : id must be provided'));
      this.app.db.query('DELETE FROM main_categories WHERE id = ?', [id], err => {
        if (err) return reject('Internal database error.');
        else resolve(true);
      });
    });
  ...
  public deleteSubCategory(id: string) {
    return new Promise((resolve, reject) => {
      if (!id) return reject(new Error('[MISSING_ARGUMENT] : id must be provided'));
      this.app.db.query('DELETE FROM sub_categories WHERE id = ?', [id], err => {
        if (err) return reject('Internal database error.');
        else resolve(true);
      });
    });
